...
void  ms_rangx(int xmin, int xmax); // Задать диапазон перемещений курсора
									// по горизонтали
void  ms_rangy(int ymin, int ymax); // Задать диапазон перемещений курсора
									// по вертикали
void  ms_gform(int xt, int yt, char _far *form); // определение формы курсора
												 // в графическом режиме
void ms_tform(int type, int mask1, int mask2);   // определение формы курсора
												 // в текстовом режиме
MOUSE_STATE *ms_querm(MOUSE_STATE *state);  // определение относительного
											// перемещения в миках
void ms_seth(int mask, void (far *hand)());  // установка драйвера событий
...
int timer(char, SYSTIMER *); // работа с часами реального времени
