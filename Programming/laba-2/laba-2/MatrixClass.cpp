...
{
	matrix C;
	if( (m!=B.m) || (n!=B.n) ) throw SizesMismatch;		//Если матрицы разного размера, генерируется исключительная ситуация
	C.SetSize(m, n);
	for (int i = 0; i < m; i++)
		for(int j = 0; j < n; j++)
...
{
	matrix C;
	if( n != B.m ) throw NotConsistent;		//Если матрицы не согласованы, генерируется исключительная ситуация
	C.SetSize(m, B.n);
	for (int i = 0; i < C.m; i++)
		for(int j = 0; j < C.n; j++)
